...
async function fetchProductData(asins, config) {
    ...
    console.log('Requesting resources:', JSON.parse(payload).Resources);
    ...
    // Log if there are any errors about resources
    if (json.Errors) {
        ...
        try {
            ...
            if (data.ItemsResult?.Items) {
                for (const item of data.ItemsResult.Items) {
                    // Debug: log full item keys for first item
                    if (i === 0 && item === data.ItemsResult.Items[0]) {
                        console.log('Sample item keys:', Object.keys(item));
                        console.log('Full first item:', JSON.stringify(item, null, 2).substring(0, 2000));
                    }
